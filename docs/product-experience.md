...
The request-cooldown-confirmation sequence begins only after the user explicitly chooses the exceptional path. Password prompts appear only when optional password protection is enabled in the Vault.
...
The Vault's add field searches by service name or hostname. Users can also type a complete hostname, such as `mail.google.com` or `account.google.com`, and choose **Add to changes** without selecting a catalog item. A friendly-name search alone is not interpreted as a hostname; users must select the matching service. Queued additions show inline feedback and remain available for review. The custom-site fields remain available for a display name and other hostname input. Filtering never silently changes a selected service's scope; filtering it out clears that unqueued selection. Password setup/replacement fields appear only when requested; current-password fields appear only when protection is enabled. Adding sites, reviewing and staging work with protection off; review, explicit confirmation and the active waiting period remain mandatory in both modes. The footer distinguishes Vault confirmation from automatically saved ordinary preferences.

**Remove from your Sphere** has a case-insensitive name/hostname search with a no-match message. Searching preserves checked removals, including those hidden by the current search; the total selection count explains this. Preview includes every selected removal, and **Clear removal selection** clears hidden selections too. Changing only the search does not invalidate a reviewed proposal or change its scope. Editing a pending proposal restores its removal selections. Core continues to validate the full batch and apply it only through the existing Vault procedure.
...
A failed load or denied redirect in a background tab retains its destination for native error presentation. Selecting that tab shows the loading-error or current Core policy boundary, not an empty browser surface labelled with the earlier allowed address. Failure cleanup uses the normal confirmed document-clearing lifecycle. Explicitly retrying an address starts a fresh native navigation even if WPF still remembers that address from a cancelled first load. A login redirect remains independently subject to site policy; this recovery does not authorize additional hosts. Returning to the Sphere dismisses the tab's failure presentation. The reproduced failure and validation are recorded in `rendering-regression.md`.
...
The Browser data card shows the running profile name and storage paths. Its temporary controls clear all browsing data, cookies, disk cache, browsing history or all site data. Each explains its scope before confirmation and closes tabs and Zenith, including for selective clearing. Cancelling leaves browsing unchanged. Site data and disk cache are labelled separately, and failures do not claim successful deletion. Storage details, limitations and manual checks are in [browser-data.md](browser-data.md).

The sidebar settings control opens a dedicated, owned native settings window. It uses the shared midnight palette and Windows High Contrast resources, with General, Your Sphere, Temporary access, Vault, Content Protection and About sections. Closing settings returns to the existing browsing surface without unloading or navigating its page.

Content Protection shows uBlock Origin Lite's browser-reported state in the current profile: Enabled, Disabled, Not installed, or Unavailable. Opening the section or selecting Refresh status obtains a new snapshot. Version information is labelled unavailable because WebView2 does not provide it. This is a status-only view; extension installation, enabling/disabling and dashboard navigation are not offered. Existing resource-filter details remain in About. See adblocking.md for the detection limits.
...
Temporary access works without password setup by default and lists pending requests with eligibility and resume actions. The native gate identifies the requested hostname, explains the explicit `www` counterpart scope, wait and confirmation using current rules and preserves the saved wait when closed. It is reached as a secondary action on an intentional Greylist boundary, with returning to the Sphere still primary. Settings → Vault offers optional shared password protection; turning it on, replacing it or turning it off uses the staged Vault procedure.
...
Settings → Temporary access includes a quiet, explicitly labelled website-address field. It accepts HTTP(S) URLs or bare website addresses such as `example.com/path`, not search queries. Omitted schemes default to HTTPS; `www.` is neither required nor automatically added. Core's explicit grant scope supports a site's redirect to its `www` counterpart after confirmation. The field continues to the existing access challenge without loading the site or starting a wait. Core decides eligibility; invalid or ineligible addresses receive inline feedback. Pending requests remain resumable below it. A separate active temporary visits section lists each requested hostname and its grant expiry, refreshing while the page is open; the scope explanation applies to these grants. Expired visits disappear; this is not persistent browsing history. This portal does not add suggestions or promote temporary destinations into ordinary discovery.
